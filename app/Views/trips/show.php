...
<div class="d-flex align-items-center mb-3 gap-2 flex-wrap">
  <h5 class="m-0">Trip <code><?= esc($row['trip_no']) ?></code></h5>
  <span class="<?= $statusCls ?>"><?= esc($row['current_status']) ?></span>
  <?php if (!empty($row['booking_id'])): ?>
    <a href="<?= site_url('bookings/' . $row['booking_id']) ?>" class="badge-soft">Booking <code><?= esc($row['booking_no']) ?></code></a>
  <?php endif; ?>
  <form method="post" action="<?= site_url('trips/' . $row['id'] . '/assign-staff') ?>" class="d-inline-flex align-items-center gap-1 ms-auto">
    <?= csrf_field() ?>
  ...
  <a class="btn btn-sm btn-light" href="<?= site_url('trips') ?>"><i class="bi bi-arrow-left"></i> Back</a>
  ...
  ?>
    <div class="btn-group btn-group-sm">
      <a class="btn btn-primary" href="<?= $lrPdf ?>" target="_blank"><i class="bi bi-file-earmark-ruled"></i> LR <?= esc($row['lr_no']) ?></a>
      <button type="button" class="btn btn-primary dropdown-toggle" data-bs-toggle="dropdown" data-bs-auto-close="outside" aria-expanded="false" title="Choose copies + download">
        <i class="bi bi-download"></i> Copies
      </button>
      ...
      <a class="btn btn-light" href="<?= site_url('dockets/from-trip/' . $row['id']) ?>" title="Edit docket"><i class="bi bi-pencil"></i></a>
    </div>
  <?php else: ?>
    <a class="btn btn-sm btn-success" href="<?= site_url('dockets/from-trip/' . $row['id']) ?>"><i class="bi bi-file-earmark-plus"></i> Create Docket</a>
  <?php endif; ?>

  <a class="btn btn-sm btn-outline-dark" href="<?= site_url('trips/' . $row['id'] . '/dispatch') ?>" title="Dispatch Pack (all LR variants)"><i class="bi bi-collection"></i> Pack</a>
  <a class="btn btn-sm btn-outline-dark" href="<?= site_url('gps/trip/' . $row['id']) ?>"><i class="bi bi-geo-alt"></i> Live Map</a>
  <a class="btn btn-sm btn-outline-dark" href="<?= site_url('invoices/from-trip/' . $row['id']) ?>"><i class="bi bi-receipt"></i> Invoice</a>
  <a class="btn btn-sm btn-outline-dark" href="<?= site_url('vendor-bills/from-trip/' . $row['id']) ?>"><i class="bi bi-journal-text"></i> Vendor Bill</a>
</div>
...
<div class="row g-3">
  <div class="col-lg-8">
    ...
    <!-- ══════════ Left-column tab strip ══════════ -->
    <ul class="nav nav-tabs mb-0" id="tripTabs" role="tablist">
      <li class="nav-item"><button class="nav-link active" type="button" data-bs-toggle="tab" data-bs-target="#tab-overview">Overview</button></li>
      <?php if (!empty($booking)): ?>
        <li class="nav-item"><button class="nav-link" type="button" data-bs-toggle="tab" data-bs-target="#tab-parties">Parties</button></li>
      <?php endif; ?>
      <li class="nav-item"><button class="nav-link" type="button" data-bs-toggle="tab" data-bs-target="#tab-dispatch">Dispatch</button></li>
      <li class="nav-item"><button class="nav-link" type="button" data-bs-toggle="tab" data-bs-target="#tab-tracking">Tracking &amp; POD</button></li>
      <li class="nav-item"><button class="nav-link" type="button" data-bs-toggle="tab" data-bs-target="#tab-expenses">Expenses</button></li>
    </ul>

    <div class="tab-content" style="background:var(--v2-surface,#fff);border:1px solid var(--v2-border,#e4e7ee);border-top:0;border-radius:0 0 var(--v2-radius,12px) var(--v2-radius,12px);padding:16px;">

      <!-- ══════════ TAB 1 — Overview: details + driver/vehicle assign ══════════ -->
        ...
        <div class="card mb-3">
          ...
          <div class="card-body">
            <div class="row g-3" style="font-size:.92rem;">
              ...
              <div class="col-md-4"><div class="text-muted">Driver</div><div><?= esc($row['driver_name'] ?? '—') ?></div></div>
              <div class="col-md-4"><div class="text-muted">Driver Mobile</div><div><?= esc($row['driver_mobile'] ?? '—') ?></div></div>
              <div class="col-md-4"><div class="text-muted">Loading Point</div><div><?= esc($row['loading_point'] ?? '—') ?></div></div>
              <div class="col-md-4"><div class="text-muted">Unloading Point</div><div><?= esc($row['unloading_point'] ?? '—') ?></div></div>
              <div class="col-md-4"><div class="text-muted">Loading Date</div><div><?= esc($row['loading_date'] ?? '—') ?></div></div>
              <div class="col-md-4"><div class="text-muted">Dispatch</div><div><?= esc($row['dispatch_datetime'] ?? '—') ?></div></div>
              <div class="col-md-4"><div class="text-muted">Delivered</div><div><?= esc($row['delivery_datetime'] ?? '—') ?></div></div>
              <div class="col-md-4"><div class="text-muted">POD</div><div><?= esc($row['pod_status']) ?><?= $row['pod_received_at'] ? ' · ' . esc($row['pod_received_at']) : '' ?></div></div>
              <?php if (!empty($booking['billing_party'])): ?>
                <div class="col-md-8"><div class="text-muted">Billing Party</div><div><?= esc($booking['billing_party']) ?></div></div>
              <?php endif; ?>
              <?php if (!empty($row['delay_reason'])): ?><div class="col-12"><div class="text-muted">Delay Reason</div><div><?= esc($row['delay_reason']) ?></div></div><?php endif; ?>
              <?php if (!empty($row['remarks'])):      ?><div class="col-12"><div class="text-muted">Remarks</div><div><?= nl2br(esc($row['remarks'])) ?></div></div><?php endif; ?>
            </div>
          </div>
        </div>
      ...
      <!-- ══════════ TAB 4 — Tracking + POD upload + e-POD signing ══════════ -->
