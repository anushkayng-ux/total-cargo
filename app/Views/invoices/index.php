<?php $exportQs = http_build_query(array_filter(['q' => $search, 'status' => $status, 'due' => $due])); ?>
<div class="d-flex align-items-center mb-3 gap-2 flex-wrap">
  <h5 class="m-0"><?= esc($pageTitle) ?></h5>
  <div data-tpt-saved-views="invoices" class="ms-2"></div>
  <form class="ms-auto d-flex gap-2" method="get" action="<?= site_url('invoices') ?>">
    <input type="text" name="q" class="form-control form-control-sm" placeholder="Search no/client/IRN" value="<?= esc($search) ?>">
    ...
    <label class="form-check form-check-inline m-0 align-self-center" style="font-size:.85rem;">
      <input type="checkbox" class="form-check-input" name="due" value="overdue" <?= $due === 'overdue' ? 'checked' : '' ?>> Overdue
    </label>
    <button class="btn btn-sm btn-outline-dark">Filter</button>
  </form>
  <a class="btn btn-sm btn-light" href="<?= site_url('invoices/export') . ($exportQs ? '?' . $exportQs : '') ?>" title="Download CSV"><i class="bi bi-download"></i> Export</a>
  <a class="btn btn-sm btn-success" href="<?= site_url('invoices/consolidated') ?>" title="Bill multiple LRs together"><i class="bi bi-collection"></i> Combined Invoice</a>
  <a class="btn btn-sm btn-primary" href="<?= site_url('invoices/create') ?>"><i class="bi bi-plus-lg"></i> New Invoice</a>
</div>
...
<div class="card">
  <div class="table-responsive">
    <table class="table mobile-cards mb-0" data-tpt-cols="invoices">
      <thead>
        <tr>
