<?php $exportQs = http_build_query(array_filter(['q' => $search, 'status' => $status])); ?>
<div class="d-flex align-items-center mb-3 gap-2 flex-wrap">
  <h5 class="m-0"><?= esc($pageTitle) ?></h5>
  <div data-tpt-saved-views="bookings" class="ms-2"></div>
  <form class="ms-auto d-flex gap-2" method="get" action="<?= site_url('bookings') ?>">
  ...
  <a class="btn btn-sm btn-light" href="<?= site_url('bookings/export') . ($exportQs ? '?' . $exportQs : '') ?>" title="Download CSV"><i class="bi bi-download"></i> Export</a>
  <a class="btn btn-sm btn-primary" href="<?= site_url('bookings/create') ?>"><i class="bi bi-plus-lg"></i> New</a>
</div>
...
<form method="post" action="<?= site_url('bookings/bulk') ?>">
  ...
  <div class="card">
    <div class="table-responsive">
      <table class="table mobile-cards mb-0" data-tpt-cols="bookings">
        <thead>
          <tr>
            ...
            <th data-col="buy" class="text-end">Buy</th>
            <th data-col="sell" class="text-end">Sell</th>
